...
--> Using TCP sockets over IPv4
--> Client calls write() to send fixed size buffer
2. What data is the client sending to the server?
-->strings from messages[] array: "hello" "Apple" "Car" "Green" "Dog"
--> each client sends NUM_MSG(5)
Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
--> pointer container: run flag to control lifetime, shared list_handle for
sorting received messages, shared list_lock mutex for synchronization
2. How are received messages stored?
-->in a linked list managed by list_handle
3. What does `main()` do with the received messages?
-->stops the acceptor, joins threads prints and free all
4. How are threads used in this sample code?
--> one acceptor thread handles incoming connections
--> one client thread per connection reads messages and appends them to list
*/
#include <arpa/inet.h>
#include <stdio.h>
#include <stdlib.h>
